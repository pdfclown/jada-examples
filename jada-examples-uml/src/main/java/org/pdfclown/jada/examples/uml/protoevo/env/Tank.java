...
import org.pdfclown.jada.examples.uml.protoevo.biology.genes.Gene;
...
import org.pdfclown.jada.examples.uml.protoevo.utils.FileIO;
    ...
    if (!hasInitialised) {
      ...
      if (Settings.initialPopulationClustering) {
        ...
        for (int j = Settings.numPopulationClusters; j < clusterCentres.length; j++) {
          ...
          float maxR =
              5 * (Settings.populationClusterRadius + Settings.populationClusterRadiusRange);
          float minR = Math.max(0.1f,
              5 * (Settings.populationClusterRadius - Settings.populationClusterRadiusRange));
