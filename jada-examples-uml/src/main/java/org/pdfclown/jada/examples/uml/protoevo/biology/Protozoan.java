...
import org.pdfclown.jada.examples.uml.protoevo.biology.genes.CAMProductionGene;
import org.pdfclown.jada.examples.uml.protoevo.biology.genes.ProtozoaGenome;
import org.pdfclown.jada.examples.uml.protoevo.biology.genes.RetinalProductionGene;
import org.pdfclown.jada.examples.uml.protoevo.core.*;
...
public class Protozoan extends Cell {
  ...
  public Protozoan(ProtozoaGenome genome, Tank tank) throws MiscarriageException {
    ...
    Map<CellAdhesion.CellAdhesionMolecule, Float> camProduction =
        genome.getGeneValue(CAMProductionGene.class);
  ...
  public float getInteractRange() {
    return retina.numberOfCells() > 0 && retina.getHealth() > 0 ? Settings.protozoaInteractRange
        : getRadius() + 0.005f;
  }
  ...
  private boolean spikeInContact(Spike spike, float spikeLen, Cell other) {
    Vector2 spikeStartPos =
        getDir().unit().rotate(spike.angle).setLength(getRadius()).translate(getPos());
