...
import org.pdfclown.jada.examples.uml.protoevo.utils.Geometry;
...
public abstract class Cell extends Particle implements Serializable {
  ...
  public Color getColor() {
    ...
    return new Color(
        (int) (healthyColour.getRed()
            + (1 - getHealth()) * (degradedColour.getRed() - healthyColour.getRed())),
        (int) (healthyColour.getGreen()
            + (1 - getHealth()) * (degradedColour.getGreen() - healthyColour.getGreen())),
        (int) (healthyColour.getBlue()
            + (1 - getHealth()) * (degradedColour.getBlue() - healthyColour.getBlue())));
  }
  ...
  public Map<String, Float> getStats() {
    ...
    float energyScalar =
        Settings.statsMassScalar * Settings.statsDistanceScalar * Settings.statsDistanceScalar;
    ...
    for (Food.ComplexMolecule molecule : complexMoleculeProductionRates.keySet())
      if (complexMoleculeProductionRates.get(molecule) > 0)
        stats.put(molecule + " Production",
            massTimeScalar * complexMoleculeProductionRates.get(molecule));

    for (Food.ComplexMolecule molecule : availableComplexMolecules.keySet())
      if (availableComplexMolecules.get(molecule) > 0)
        stats.put(molecule + " Available",
            100f * Settings.statsMassScalar * availableComplexMolecules.get(molecule));
  ...
  public void resourceProduction(float delta) {
    for (Food.ComplexMolecule molecule : complexMoleculeProductionRates.keySet()) {
      ...
      if (producedMass > 0 && constructionMassAvailable > producedMass
  ...
  private void handleComplexMoleculeTransport(Cell other, Food.ComplexMolecule molecule,
      float delta) {
    float massDelta =
        getComplexMoleculeAvailable(molecule) - other.getComplexMoleculeAvailable(molecule);
