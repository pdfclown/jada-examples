...
import org.pdfclown.jada.examples.uml.protoevo.biology.NNBrain;
import org.pdfclown.jada.examples.uml.protoevo.biology.Protozoan;
import org.pdfclown.jada.examples.uml.protoevo.core.Application;
...
import org.pdfclown.jada.examples.uml.protoevo.neat.NeuralNetwork;
import org.pdfclown.jada.examples.uml.protoevo.neat.Neuron;
import org.pdfclown.jada.examples.uml.protoevo.ui.Window;
import org.pdfclown.jada.examples.uml.protoevo.ui.components.TextButton;
import org.pdfclown.jada.examples.uml.protoevo.ui.components.TextObject;
import org.pdfclown.jada.examples.uml.protoevo.ui.components.TextStyle;
import org.pdfclown.jada.examples.uml.protoevo.ui.components.UIComponent;
import org.pdfclown.jada.examples.uml.protoevo.utils.*;

/**
 * @author Dylan Cope
 */
public class SimulationUI implements ChangeListener {
  private final org.pdfclown.jada.examples.uml.protoevo.ui.Window window;
  ...
  public SimulationUI(Window window, Simulation simulation, SimulationRenderer renderer) {
    ...
    TextButton pauseButton =
        new TextButton(new TextObject("Pause", infoTextSize, Color.WHITE.darker()));
    ...
    TextButton speedButton =
        new TextButton(new TextObject("x1", infoTextSize, Color.WHITE.darker()));
    ...
    for (int depth = 0; depth <= networkDepth; depth++) {
      ...
      for (Neuron n : neurons) {
        if (n.getDepth() == depth && n.isConnectedToOutput()) {
          int y = (int) (boxYStart + nodeSpacing / 2f + boxHeight / 2f
              - (nNodes / 2f - i) * nodeSpacing);
    ...
    if (boxXStart - 2 * r < mouseX && mouseX < boxXStart + boxWidth + 2 * r &&
      ...
      for (Neuron neuron : nn.getNeurons()) {
        ...
        if (neuron.getLabel() != null &&
          ...
          String labelStr =
              neuron.getLabel() + " = " + TextStyle.numberToString(neuron.getLastState(), 2);
