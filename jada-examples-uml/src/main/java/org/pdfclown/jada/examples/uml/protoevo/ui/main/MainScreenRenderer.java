...
public class MainScreenRenderer extends Canvas {
  public static void main(String[] args) {
    TextStyle.loadFonts();
    org.pdfclown.jada.examples.uml.protoevo.ui.Window window = new Window("Evolving Protozoa");
  ...
  public MainScreenRenderer(Window window) {
    ...
    title.setPosition(
        new Vector2(window.getWidth() / 2f - title.getWidth() / 2f, window.getHeight() / 8f));
    ...
    if (saves != null && saves.length > 0) {
      ...
      float newSimY =
          window.getHeight() / 2f - newSimulationButtonText.getHeight() / 2f - nSaves * offset / 2f;
          ...
          TextButton loadSaveButton = new TextButton(
              ...
              () -> {
                ...
                SimulationController controller =
                    new SimulationController(window, simulation, renderer);
                window.set(renderer, controller);
